                        ...
                        <div class="d-flex flex-column justify-content-between gap-4 gap-md-6 h-100">
                            ...
                            <div class="mid-area py-10 d-grid gap-6 gap-md-10">
                                <div class="head-area d-grid gap-3 gap-md-5">
                                    <h2 class="n17-color">Profile</h2>
                                    <p class="n17-color fs-six">Update your profile information.</p>
                                </div>
                                @if (session('success'))
                                ...
                                @endif
                                <form method="POST" action="{{ route('profile.update') }}" class="row gy-4 gy-md-6">
                                    @csrf
                                    <div class="col-sm-12 d-grid gap-1 gap-md-2">
                                        <label for="name" class="fw-mid fs-six">Name</label>
                                        <div class="input-area w-100 n1-bg-color rounded-pill transition cus-border border">
                                            <input type="text" id="name" name="name" value="{{ auth()->user()->name }}" class="px-4 px-md-6 py-2 py-md-3 w-100 rounded-pill" required autofocus>
                                        </div>
                                        @error('name')
                                    ...
                                    <div class="col-sm-12 d-grid gap-1 gap-md-2">
                                        ...
                                        <div class="input-area w-100 n1-bg-color rounded-pill transition cus-border border">
                                            <input type="email" id="email" name="email" value="{{ auth()->user()->email }}" class="px-4 px-md-6 py-2 py-md-3 w-100 rounded-pill" required>
                                        </div>
                                        @error('email')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    ...
                                    <div class="col-sm-12 d-grid gap-6 gap-md-10">
                                        <button type="submit" class="box-style box-second gap-2 gap-md-3 rounded-pill py-5 py-md-6 px-5 px-md-7 d-center">
                                            <span class="position-absolute show transition n1-color fw-mid">Update Profile</span>
                                        </button>
                                    </div>
                                </form>
                            </div>
