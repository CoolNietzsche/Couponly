                ...
                <div class="row cus-row top-stores trending-features g-3">
                    ...
                    <div class="col-6 col-sm-4 col-lg-3 col-xxl-2">
                        <a href="{{ route('coupons.index', ['category' => $category->slug]) }}" 
                           class="single-box transition d-grid gap-3 gap-md-4 text-center n1-bg-color cus-border border b-seventh px-2 px-xxl-3 py-4 py-xxl-6 rounded-2">
                            
                            <div class="d-center thumb-area rounded-2 w-100">
                                <img loading="lazy" class="max-un" src="{{ asset($category->icon) }}" alt="{{ $category->name }}">
                            </div>

                            <div class="text-area">
                                <span class="n17-color fs-five fw-mid d-block">{{ $category->name }}</span>
                                <span class="s1-color fs-seven">{{ $category->coupons_count }} Active Deals</span>
                            </div>
                        </a>
                    </div>
                    @endforeach
